function initPawMask = find_initPawMask_tattooPaw( video, BGimg_ud, sr_ratInfo, session_mp, boxCalibration, boxRegions, triggerTime, varargin )
...
decorr_red_BG = decorrstretch(BGdiff_stretch,...
                             'targetmean',targetMean(1,:),...
                             'targetsigma',targetSigma(1,:));
decorr_red_BG_hsv = rgb2hsv(decorr_red_BG);
...
redHSVthresh = HSVthreshold(decorr_red_BG_hsv, pawHSVrange(1,:));
...
redMasked = im_masked & redHSVthresh;
...
leftMirrorRed = leftMirrorMask & redMasked;
rightMirrorRed = rightMirrorMask & redMasked;
directRed = directMask & redMasked;

mirrorRed = cell(1,2);
mirrorRed{1} = processMask(leftMirrorRed,2);
mirrorRed{2} = processMask(rightMirrorRed,2);
directRed = processMask(directRed,2);

% if mirror green masks are adjacent to the front panel, look for green
% behind the front panel
for iSide = 1 : 2
    overlapMask = imdilate(mirrorRed{iSide},strel('disk',5)) & frontPanelMask{iSide};
    if any(overlapMask(:))
        mirrorRed{iSide} = mirrorRed{iSide} | (frontPanelEdge{iSide} & intMask & redHSVthresh);
    end
    mirrorRed{iSide} = processMask(mirrorRed{iSide}, 2);
end
switch pawPref
    case 'left',
        mirror_mask = mirrorRed{2};    % right mirror
        fundMat = srCal.F(:,:,2);
    case 'right',
        mirror_mask = mirrorRed{1};     % left mirror
        fundMat = srCal.F(:,:,1);
end
direct_mask = directRed;
...
mirror_proj_overlap = (mirror_mask & mirror_projMask);
direct_proj_overlap = (direct_mask & direct_projMask);
