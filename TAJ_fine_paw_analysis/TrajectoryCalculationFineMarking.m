...
%Input
%Will load the data from the fine marking 3d analysis that is already
%seperated into the success and failure reaches


%Rat ID
%Figure with all reaches
%Figure with just the average trajectories
%The overall sucess rate
...
%Output
%Will spit out the averaged trajectories for the given reaches 


function  [allCentroids,averagedCentroids, euclidianDistDiff,  euclidianDistDiffMean, euclidianDistDiffStd,averagedCentroidsDisp]= TrajectoryCalculationFineMarking(all3dPoints,score,ratID,sessionDate,fig_num_all,fig_num_avg)%,fig_num_all,sucessRate,totalNumReaches)


    for i = 1:length(all3dPoints(:,1))
        for j =1:length(all3dPoints(1,:))
            
            if isempty(all3dPoints{i,j}) ~= 1
    ...
    averagedCentroids = averageCentroids(allCentroids,score,fig_num_avg)%,sucessRate,RatID,totalNumReaches);
   ...
   [averagedCentroidsDisp] = calculatePositionChange3D(averagedCentroids)
    ...
    plotCentroidTrajectories(allCentroids,fig_num_all,score)%,day,sucessRate,RatID,totalNumReaches);
    
    

end

%% Calculate the centroid from a given frame
...
%% Average Centroids from all centroids
function  [averagedCentroids] = averageCentroids(allCentroids,score,fig_num_avg)%,sucessRate,RatID,totalNumReaches)
...
z_std = [];
           ...
           for k = 1:length(averagedCentroids)
...
%% Calculate variance in reaching
...
averagedCentroidsDisp= [];
        ...
        for k =1:(length(averagedCentroids(1,:))-1) %number of frames
